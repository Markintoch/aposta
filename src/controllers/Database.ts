...
class Database {
    ...
    async deleteById( tabla : string, atributo : string, atributo_id : any ){
        ...
        await this.runQueryAsync( deleteQuery, deleteData ).catch( (error : any) => { 
            if(error.code == 23503){ throw new Error(Messages.FOREIGN_KEY_VIOLATION); }
            else{ throw new Error(Messages.QUERY_DELETE_ERROR); }
        } );
    }
    ...
    async selectJornadasWeek( idLiga : any, idTemporada : any ){
        let selectQuery = `SELECT j.*, l.nombre as "nombre_liga", t.nombre || '(' || t.numero || ')' as "nombre_temporada" FROM jornadas j INNER JOIN temporadas t on t.temporada_id = j.temporada_id INNER JOIN ligas l on l.liga_id = j.liga_id`;
    ...
    async selectTemporadas( idLiga : any = null ){
        ...
        if(idLiga) selectQuery = selectQuery + ` ${!selectQuery.includes('WHERE') && 'WHERE'} l.liga_id = ${idLiga}`
    ...
    async selectTemporadasMin( idLiga : any = null ){
        let selectQuery = `SELECT t.temporada_id,  t.nombre || '(' || t.numero || ')' as "nombre_temporada" FROM temporadas t`;

        if(idLiga) selectQuery = selectQuery + ` ${!selectQuery.includes('WHERE') && 'WHERE'} t.liga_id = ${idLiga}`
