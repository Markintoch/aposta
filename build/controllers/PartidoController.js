...
class Partido {
    ...
    async getPartidos(request, response) {
        try {
            ...
            let resultQuery = await Database_1.DatabaseController.selectPartidos(Number(idLiga), Number(idTemporada), Number(idJornada));
            let body = { status: 200, data: resultQuery };
            response.json(body);
        }
        catch (error) {
    ...
    async getPartidosByLiga(request, response) {
        try {
            let liga_id = request.params.id;
            if (liga_id == undefined || liga_id == null) {
                throw new Error(messages_1.Messages.LIGA_ID_ISREQUIRED);
            }
            let resultQuery = await Database_1.DatabaseController.selectPartidosByLiga(liga_id); //simpleSelectById( "partidos", "jornada_id", jornada_id );
            let body = { status: 200, data: resultQuery };
            response.json(body);
        }
        catch (error) {
    ...
    async getPartidosByTemp(request, response) {
        try {
            let temporada_id = request.params.id;
            if (temporada_id == undefined || temporada_id == null) {
                throw new Error(messages_1.Messages.LIGA_ID_ISREQUIRED);
            }
            let resultQuery = await Database_1.DatabaseController.selectPartidosByTemp(temporada_id); //selectPartidosByLiga(liga_id); //simpleSelectById( "partidos", "jornada_id", jornada_id );
            let body = { status: 200, data: resultQuery };
            response.json(body);
        }
        catch (error) {
    ...
    async getPartidosByJornada(request, response) {
        try {
            ...
            let resultQuery = await Database_1.DatabaseController.selectPartidosByJornada(jornada_id); //selectPartidosByLiga(liga_id); //simpleSelectById( "partidos", "jornada_id", jornada_id );
            let body = { status: 200, data: resultQuery };
            response.json(body);
        }
        catch (error) {
